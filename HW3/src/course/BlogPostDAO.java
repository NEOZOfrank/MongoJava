...
public class BlogPostDAO {
	...
	public List<DBObject> findByDateDescending(int limit) {
		...
		//  HW 3.2, Work Here
		// Return a list of DBObjects, each one a post from the posts
		// collection

		BasicDBObject query = new BasicDBObject();

		DBCursor cursor = postsCollection.find(query).sort(new BasicDBObject("date", -1)).limit(limit);
	...
	public String addPost(String title, String body, List tags, String username) {
		...
		// - tags are already in list form that implements suitable
		// interface.
		...
		post.append("title", title).append("author", username).append("body", body).append("permalink", permalink)
				.append("tags", tags).append("comments", new String[] {}).append("date", new Date());
	...
	public void addPostComment(final String name, final String email, final String body, final String permalink) {
		...
		// - best solution uses an update command to the database and a
		// suitable
		// operator to append the comment on to any existing list of
		// comments
